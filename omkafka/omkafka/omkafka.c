...
free_topic(rd_kafka_topic_t **topic)
{
  ...
  /* TODO */
  destroy_writer(avro_writer, (char **)&avro_buffer);
  DBGPRINTF("omkafka: avro writer destroyed\n");
...
/* Get a new topic conf */
  ...
  /* TODO */
  if(init_writer(&avro_writer, &avro_buffer, AVROBUFSIZE) != 0){
		LogError(0, RS_RET_KAFKA_ERROR, "omkafka: error initialising avro writer\n");
  }
  else {
    DBGPRINTF("omkafka: avro writer initialised\n");
  }
	...
	uchar *const msgTimestamp, uchar *const topic, const int b_do_resubmit)
{
  ...
  /* TODO */
  int size = convert_to_avro(avro_writer, (const char *)msg);
  if(size == 0) {
	  LogError(0, RS_RET_KAFKA_PRODUCE_ERR,"omkafka: could not convert to avro schema\n");
  }
  else {
    DBGPRINTF("omkafka: avro size:%i, msg: %s\n", size, avro_buffer);
    /* reset writer buffer to 0 */
	...
	if (key == NULL) {
		msg_kafka_response = rd_kafka_producev(pData->rk,
						...
						RD_KAFKA_V_VALUE(avro_buffer, size),//RD_KAFKA_V_VALUE(msg, strlen((char*)msg)),
