...
//DBGPRINTF("imkafka: enqMsg: Msg: %.*s\n", (int)rkmessage->len, (char *)rkmessage->payload);
   ...
   /*TODO */
  char * msg_t = "<34>1 2003-10-11T22:14:15.003Z test.local app - ID42 - data\n";//pointer to result msg
  char * msg = NULL;
  size_t msg_size = 0;
  convert_from_avro(rkmessage->payload, rkmessage->len, &msg, &msg_size);	//MsgSetRawMsg(pMsg, (char*)rkmessage->payload, (int)rkmessage->len);
  DBGPRINTF("imkafka: avro msg = %s, size = %i\n", msg, (int)msg_size);
  MsgSetRawMsg(pMsg, msg, msg_size);//MsgSetRawMsg(pMsg, msg_t, strlen(msg_t));
  ...
  if(msg) {
    free(msg);//free result msg after submitting
  }
